...
const Customers = () => {
  ...
  const loadCustomers = async () => {
    try {
      ...
      setCustomers((prev) => {
        // Se houve mudança de conteúdo (ex.: acabou de criar), volta pra página 1
  ...
  // --- índice original para fallback estável (últimos carregados primeiro)
  ...
  // Filtra e ORDENA (DESC por createdAt) **antes** da paginação
  const filteredCustomers = useMemo(() => {
    ...
    // 2) ordena (DESC) por: createdAt → ObjectId → índice carregado → nome
    const getCreatedAtMs = (obj) => {
      const ts = obj?.createdAt ?? obj?.created_at ?? null;
      if (ts) {
        const t = Date.parse(ts);
        if (Number.isFinite(t)) return t;
      }
      // Fallback: se id for ObjectId (24 hex), usa o timestamp embutido
    ...
    return [...base].sort((a, b) => {
      ...
      // prioridade: createdAt desc se existir ao menos uma válida
      if (!Number.isNaN(tb) || !Number.isNaN(ta)) {
        if (Number.isNaN(ta)) return -1; // b tem data; a não
        if (Number.isNaN(tb)) return 1;  // a tem data; b não
        if (tb !== ta) return tb - ta;   // ambos com data
      }
      // fallback: último carregado primeiro
  ...
  }, [customers, debouncedSearch, indexMap]);
