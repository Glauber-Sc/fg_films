...
export default function PDVPro() {
  ...
  // >>> DRAFT LOCAL PARA QUANTIDADE (permite apagar e digitar livre)
  const [qtyDraft, setQtyDraft] = useState({}); // { [id]: string }

  const commitQty = (id, str) => {
    const n = Math.max(1, Number((str ?? "").trim() || "1"));
    dispatch({ type: "UPDATE_QTY", payload: { id, qty: n } });
    setQtyDraft((d) => {
      const x = { ...d };
      delete x[id];
      return x;
  ...
  const handleFinalize = async () => {
    ...
    try {
      ...
      // imprime recibo A5 (se habilitado)
      if (state.printReceipt) {
        paymentReceipt(payload, { logoUrl: logo });
      }

      // >>> ALTERAÇÃO PEDIDA: voltar para o step "produto" após finalizar
      setCurrentStep("produto");
      // opcional: focar o campo de busca imediatamente
      setTimeout(() => searchRef.current?.focus(), 0);

      // limpar estado e atualizar lista de produtos
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100dvh", overflow: "auto", background: C.bgApp }}>
      ...
      >
        {/* slot fixo do header (mantém 50px de altura) */}
        <div style={{ height: 50, width: 150, position: "relative", overflow: "visible" }}>
          <img
            src={logo}
            alt="Venda-PRO"
            style={{
                    ...
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                      ...
                      >
                        <button
                          type="button"
                          onClick={() => {
                            setQtyDraft((d) => { const x = { ...d }; delete x[it.id]; return x; });
                            dispatch({ type: "UPDATE_QTY", payload: { id: it.id, qty: Math.max(1, it.quantity - 1) } });
                          }}
                          style={{
                            width: 34,
                            height: 30,
