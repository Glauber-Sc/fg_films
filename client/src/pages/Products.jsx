...
const Products = () => {
  ...
  const handleBulkDelete = async () => {
    ...
    if (!window.confirm(`Excluir ${ids.length} produto(s) selecionado(s)?`))
      return;
    try {
      setIsLoading(true);
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      XLSX.utils.sheet_add_aoa(ws, [cols.map((c) => headerMap[c])], {
        origin: "A1",
      });
  ...
  const getThreshold = (p) =>
    Number.isFinite(p?.lowStock) ? Number(p.lowStock) : 5;
    ...
    <div className="p-6">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500">
          Gerencie o catálogo e monitore o nível de estoque
        </p>
      </div>
      ...
      <div className="overflow-hidden rounded-lg bg-white shadow">
        <div className="flex items-start justify-between border-b bg-gray-50 px-6 py-4">
          <div>
            ...
            <p className="text-sm text-gray-500">
              Resumo dos produtos{" "}
              {activeTab === "low" ? "com baixo estoque" : "cadastrados"}
            </p>
          </div>
                    ...
                    return (
                      <tr key={product.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 text-sm text-gray-900">
                          <span
                            className={`${
                              Number(product.stock) <= getThreshold(product)
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                          <div className="flex items-center gap-2">
                            <Link
                              to={`/products/view/${encodeURIComponent(
                                product.id
                              )}`}
