...
// label PT-BR para o tipo
const typeLabel = (t = "") => {
  const k = String(t).toLowerCase();
  if (k === "sale" || k === "venda") return "Venda";
  if (k === "service" || k === "servico" || k === "serviço") return "Serviço";
  if (k === "expense" || k === "despesa") return "Despesa";
  return t || "—";
};
    ...
    <div className="bg-white border border-gray-100 shadow rounded-md p-5">
      ...
      {pct === null ? (
        ...
        <div className="mt-1 flex items-center text-sm">
          ...
          <span
            className={up ? "text-green-600 font-medium" : "text-red-600 font-medium"}
          >
            {fmtPct(pct)}
          </span>
...
export default function Reports() {
  ...
  // Saldo acumulado calculado em ordem cronológica (crescente)
  ...
  // ===== NOVO: apenas a exibição em ORDEM DECRESCENTE =====
  const ledgerDesc = useMemo(() => {
    return [...ledgerView].sort(
      (a, b) =>
  ...
  // ===== paginação (usa a lista desc) =====
  const [pageSize, setPageSize] = useState(10);
  const [page, setPage] = useState(1);
  useEffect(() => setPage(1), [ledgerDesc, pageSize, typeView]);
  const totalItems = ledgerDesc.length;
  const totalPages = Math.max(1, Math.ceil(totalItems / pageSize));
  const safePage = Math.min(Math.max(1, page), totalPages);
  const startIdx = (safePage - 1) * pageSize;
  const endIdx = Math.min(startIdx + pageSize, totalItems);
  const rows = ledgerDesc.slice(startIdx, endIdx);
    ...
    <div className="p-6">
      ...
      <div className="border-b bg-white/80 backdrop-blur supports-[backdrop-filter]:bg-white/70">
        <div className="px-3 py-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* Export CSV (continua cronológico) */}
            <button
              type="button"
              onClick={() => {
                setExporting(true);
                try {
                  ...
                  // exporta em ordem cronológica (ledgerView)
      ...
      <div className="mt-4 grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <MetricCard
          ...
          label={trendLabelFromMode(filters.mode)}
          valueClassName={netValue >= 0 ? "text-emerald-600" : "text-red-600"}
        />
      </div>
          ...
          <div className="flex items-start justify-between border-b bg-gray-50 px-6 py-4">
            ...
            <div className="text-right">
              ...
              <div className="text-sm text-gray-700">
                ...
                <span>
                  Saldo:{" "}
                  <strong className={viewTotals.net < 0 ? "text-red-600" : "text-gray-900"}>
                    {formatCurrency(viewTotals.net)}
                  </strong>
                </span>
              </div>
              <p className="text-xs text-gray-500 mt-1">
                Exibindo {totalItems === 0 ? 0 : startIdx + 1}–{endIdx} de {totalItems}
              </p>
            </div>
          </div>
